...
if (!isset($_POST['username'], $_POST['password'], $_POST['cpassword'], $_POST['email'])) {
	...
	exit('Error: Kérjük, töltsd ki a regisztrációs űrlapot!');
}
...
if (!filter_var($_POST['email'], FILTER_VALIDATE_EMAIL)) {
	exit('Error: Kérjük, adj meg egy érvényes e-mail címet!');
}
// Username must contain only characters and numbers.
if (!preg_match('/^[a-zA-Z0-9]+$/', $_POST['username'])) {
    exit('Error: A felhasználónév csak betűket és számokat tartalmazhat!');
}
// Password must be between 5 and 20 characters long.
if (strlen($_POST['password']) > 20 || strlen($_POST['password']) < 5) {
	exit('Error: A jelszónak 5 és 20 karakter között kell lennie!');
}
// Check if both the password and confirm password fields match
if ($_POST['cpassword'] != $_POST['password']) {
	exit('Error: A jelszavak nem egyeznek!');
}
...
if ($stmt->num_rows > 0) {
	...
	echo 'Error: Már létezik ilyen felhasználónévvel vagy e-mail címmel fiók!';
} else {
	$stmt->close();
	...
	if (account_activation) {
		...
		echo 'Success: Kérjük, ellenőrizd az e-mail fiókodat a fiók aktiválásához!';
	} else {
		...
		if (auto_login_after_register) {
			...
			echo 'Success: Sikeres regisztráció! Most már bejelentkezhetsz!';
		}
	}
}
?>